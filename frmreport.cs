...
namespace LabManagementApp
{
    public partial class frmreport : Form
    {
        ...
        List<string> AllListTimeSlot;
        
        private List<string> GetTimeSlot()
        {
            List<string> lstTimeslot= new List<string>();
            try
            {
                lstTimeslot = objdb.GetTimeSlot().AsEnumerable().Select(x => x.Field<string>("slot")).ToList();
            }
            catch (Exception ex)
            {
                objdb.WriteLog();
           ...
           // dtp_etime.Text = DateTime.Now.TimeOfDay.ToString();
            try
            {
                ...
                using (DataTable dt = objdb.GetExprimentReports())
                {
                    ...
                    dg_report.Columns[7].HeaderText = "Start Date";
                    dg_report.Columns[7].DataPropertyName = "startdate";

                    dg_report.Columns[8].HeaderText = "Slot Time";
        ...
        private void btn_insert_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (cmboTimeslot.SelectedValue.ToString() == "No slot available")
                    {
                        MessageBox.Show("Time Slot Not Available!", "Time Slot", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }
                   ...
                   // string edate = dtp_edate.Text.Trim() + dtp_etime.Text.Trim();
                    string sdate = dtp_timeslot.Text.Trim(); 
                    string edate = cmboTimeslot.SelectedValue.ToString();
        ...
        private void tsm_addnew_Click(object sender, EventArgs e)
        {
            ...
            {
                clearAll();
                if (cmboTimeslot.SelectedText == "No slot available")
                {
                    MessageBox.Show("Time Slot Not Available!", "Time Slot", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
        ...
        private void tsm_edit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dg_report.DataSource != null && dg_report.SelectedRows.Count > 0)
                {
                    lbl_msg.Visible = true;
                    cmboTimeslot.DataSource = null;
                    cmboTimeslot.DataSource = AllListTimeSlot;
                    txt_name.Text = dg_report.SelectedRows[0].Cells[1].Value.ToString();
                    txt_organization.Text = dg_report.SelectedRows[0].Cells[2].Value.ToString();
                    txt_visitorname.Text = dg_report.SelectedRows[0].Cells[3].Value.ToString();
                    txt_projectno.Text = dg_report.SelectedRows[0].Cells[4].Value.ToString();
                   ...
                   // dtp_sdate.Text = dg_report.SelectedRows[0].Cells[7].Value.ToString();
                    //dtp_edate.Text = dg_report.SelectedRows[0].Cells[8].Value.ToString();
                   // dtp_stime.Text = dg_report.SelectedRows[0].Cells[7].Value.ToString().Substring(10, 11);
                    dtp_timeslot.Text = dg_report.SelectedRows[0].Cells[7].Value.ToString();
                   // ListTimeSlot = objdb.GetTimeSlot().AsEnumerable().Select(x => x.Field<string>("slot")).ToList();
                    string slotval = dg_report.SelectedRows[0].Cells[8].Value.ToString();
                    cmboTimeslot.SelectedIndex = AllListTimeSlot.FindIndex(a => a.Equals(slotval));
                    txt_description.Text = dg_report.SelectedRows[0].Cells[9].Value.ToString();
                    txt_email.Text = dg_report.SelectedRows[0].Cells[10].Value.ToString();
                    txt_phone.Text = dg_report.SelectedRows[0].Cells[11].Value.ToString();
                    oldreportFile = dg_report.SelectedRows[0].Cells[13].Value.ToString();
        ...
        private void clearAll()
        {
            try
            {
                ...
                // dtp_stime.Text = DateTime.Now.TimeOfDay.ToString();
                //dtp_etime.Text = DateTime.Now.TimeOfDay.ToString();
                dtp_timeslot.Text = DateTime.Now.ToString();
                //cmboTimeslot.DataSource = objdb.GetTimeSlot().AsEnumerable().Select(x => x.Field<string>("slot")).ToList();
                ...
                try
                {
                    if (dg_download.RowCount == 0)
                    {
                       ...
                       // dg_report.Columns[1].Width = 200;
                        ...
                        dg_download.Columns[7].HeaderText = "Start Date";
                        dg_download.Columns[7].DataPropertyName = "startdate";

                        dg_download.Columns[8].HeaderText = "Slot Time";
        ...
        private List<string> GetUpdatedTimeSlot(string date)
        {
            List<string> lsttime=new List<string>();
            try
            {

                List<string> dbtimeslot = objdb.GetExprimentReports().AsEnumerable().Where(x => x.Field<string>("startdate").Equals(date)).
                      
                                                                                          Select(y => y.Field<string>("enddate")).ToList();
              // lsttime=ListTimeSlot.Remove()
               // foreach (var p in GetTimeSlot())
                //{
                   ListTimeSlot = GetTimeSlot();
                    foreach (var n in dbtimeslot)
                    {
                        //if (!lsttime.Contains(p))
                        //if (n != p)
                         //   lsttime.Add(p);
                        
                        ListTimeSlot.Remove(n);
                    }
                    if (ListTimeSlot.Count == 0)
                        lsttime.Add("No slot available");
                    else
                    lsttime = ListTimeSlot;
              //  }
